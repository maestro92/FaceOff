...
{
	PEParticle particles[MAX_PARTICLES];
...
bool ParticleEffect::initRandomTexture(int size)
{
...
void ParticleEffect::update(Pipeline& p, Renderer* r)
{
	...
	p.pushMatrix();
		...
		glVertexAttribPointer(0, 1, GL_FLOAT, GL_FALSE, sizeof(PEParticle), 0);								// type
		glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(PEParticle), (const GLvoid*)4);				// position
		glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(PEParticle), (const GLvoid*)16);				// velocity
		glVertexAttribPointer(3, 1, GL_FLOAT, GL_FALSE, sizeof(PEParticle), (const GLvoid*)28);				// lifetime
...
void ParticleEffect::render(Pipeline& p, Renderer* r)
{
	...
	//	glDisable(GL_RASTERIZER_DISCARD);
		...
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(PEParticle), (const GLvoid*)4);  // position
